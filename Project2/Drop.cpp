...
// load the texture.
bool
Drop::Initialize(void)
{
	int i, n = 10;

	seat_list = glGenLists(1);
	glNewList(seat_list, GL_COMPILE);
	glColor3f(1.0, 1.0, 1.0);

	object.loadOBJ("seat.obj");
	...
	object2.loadOBJ("tower.obj");

	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, textureId);
	LoadTexture("darkstrips.tga");
...
void Drop::LoadTexture(const char* filename)
{
...
bool Drop::ResizeImage(TargaImage* image)
{
